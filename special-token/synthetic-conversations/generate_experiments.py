...
# 1) Reading base personas + styles from config/personas.yaml
...
#    - init_user_message (first user turn) K times per (base_persona, style)
...
def make_persona_id(base_key: str, style_key: str, init_idx: int) -> str:
    return f"{base_key}__{style_key}__init{init_idx}"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--num_init_prompts", type=int, default=1, help="init_user_message samples per profile")
...
def main() -> None:
    ...
    banned = collect_banned_strings(personas_cfg, list(base_map), list(style_map))
    ...
    print(f"[config] base={len(base_items)} style={len(style_items)} num_init_prompts={args.num_init_prompts}")
    ...
    with out_path.open("a", encoding="utf-8") as f_out:
        for base_key, base_text in base_items:
            for style_key, style_text in style_items:
                ...
                # generate init_user_message K times
                for init_idx in range(args.num_init_prompts):
                    persona_id = make_persona_id(base_key, style_key, init_idx)
