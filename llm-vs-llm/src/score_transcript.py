# src/score_transcript.py
#
# Minimal scorer for transcripts where investigator_mode == "none".
# Produces per-assistant-turn NLL scores and saves to data/scores/*.parquet.
#
...
#   - system_llm2 (assistant system prompt)
...
def build_inputs_for_assistant_turn(
    ...
    prompt_ids = apply_chat_template(tokenizer, prompt_msgs, add_generation_prompt=True)

    full_msgs = list(prompt_msgs) + [{"role": "assistant", "content": assistant_text}]
...
def iter_assistant_turns(messages: List[Dict[str, str]]) -> List[Tuple[int, List[Dict[str, str]], str]]:
    ...
    for m in messages:
        ...
        if role == "assistant":
            turns.append((a_idx, list(history), content))
            a_idx += 1
            history.append({"role": "assistant", "content": content})
        elif role == "user":
            history.append({"role": "user", "content": content})
        else:
            # ignore other roles in transcript messages (system usually not here)
            continue
...
def score_transcript_inv_none(
    ...
    for i, row in enumerate(rows):
        ...
        for turn_idx, history_before, assistant_text in turns:
            s = score_one_assistant_turn(tok, model, system_llm2, history_before, assistant_text)
            records.append({
                "conversation_idx": i,
    ...
    out_path = out_dir / f"{transcript_path.stem}__inv_none__per_turn.parquet"
    df.to_parquet(out_path, index=False)
    return out_path


def main() -> None:
    out_path = score_transcript_inv_none(TRANSCRIPT_PATH, OUT_DIR, SCORER_MODEL)
